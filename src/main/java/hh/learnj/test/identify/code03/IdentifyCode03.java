package hh.learnj.test.identify.code03;
 ...
 * 
 * 前面的验证码背景都比较简单，用亮度稍微区分一下就可以去掉背景 来看个稍微复杂一点的
 * 1。图片预处理 怎么去掉背景干扰呢。 可以注意到每个验证码数字或字母都是同一颜色，所以把验证码平均分成5份
 * 计算每个区域的颜色分布，除了白色之外，颜色值最多的就是验证码的颜色 因此很容易将背景去掉
 * @author Hunny.Hu
 */
public class IdentifyCode03 {
