...
//			System.setProperty("java.library.path", "so");
			property();
			String dllPath = "sigar-amd64-winnt.dll";  
			String dllName = "sigar-amd64-winnt.dll";
	...
	private static void cpu() throws SigarException {
		...
		for (int i = 0; i < infos.length; i++) {// 不管是单块CPU还是多CPU都适用
			...
			System.out.println("CPU Map:    " + info.toMap());// 
	...
	private static void setLibrary(String dllPath, String dllName) {
		try {
			...
			// 把dll文件写入到java.library.path中该dll放在相同目录下，这个可以是你的类名
			InputStream inputStream = RuntimeTest.class.getResourceAsStream(dllPath);
			...
			e.printStackTrace();
			System.err.print("load dll error " + e.getMessage());
		}
	}
}
