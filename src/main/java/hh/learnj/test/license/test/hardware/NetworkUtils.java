...
public final class NetworkUtils {
	...
	private final static String windowsParseMacAddress(String ipConfigOutput) throws ParseException {
		...
		while (tokenizer.hasMoreTokens()) {
			...
			String valueCandidate = line.substring(valuePosition + 1).trim();
			if (windowsIsMacAddress(valueCandidate)) {
				lastMacAddress = valueCandidate;
				continue;
			}
			// see if line contains IP address
			if ((valueCandidate.endsWith(localHost) || valueCandidate.startsWith(localHost)) && lastMacAddress != null) {
				return lastMacAddress;
			}
		}
	...
	private final static String windowsIpConfigCommand() throws IOException {
		...
		int count = -1;
		while ((count = stdoutStream.read(data, 0, 4096)) != -1) {
