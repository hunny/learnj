 ...
 * 
 */
package hh.learnj.test.license.test.coder;
...
public class RSACoderTest {
	...
	/*
	 * 非对称加密算法 RSA过程 ： 以甲乙双方为例 1、初始化密钥 构建密钥对,生成公钥、私钥保存到keymap中 KeyPairGenerator
	 * ---> KeyPair --> RSAPublicKey、RSAPrivateKey 2、甲方使用私钥加密,
	 * 加密后在用私钥对加密数据进行数据签名，然后发送给乙方 RSACoder.encryptByPrivateKey(data,
	 * privateKey); RSACoder.sign(encodedData, privateKey);
	 * 3、乙方则通过公钥验证签名的加密数据，如果验证正确则在通过公钥对加密数据进行解密 RSACoder.verify(encodedData,
	 * publicKey, sign); RSACoder.decryptByPublicKey(encodedData, publicKey);
	 * 
	 * 4、乙方在通过公钥加密发送给甲方 RSACoder.encryptByPublicKey(decodedData, publicKey);
	 * 5、甲方通过私钥解密该数据 RSACoder.decryptPrivateKey(encodedData, privateKey);
	 */
	@Before
